...
- private 인스턴스 속성은 __double_underscore 처럼 밑줄 두개로 시작한다
...
* 왈러스 연산자 `:=`(대입식): :bangbang: &nbsp; __파이썬 3.8(이상)__ 에서 새로 도입된 구문
  * 왈러스(walrus) == 바다코끼리
* __대입식__ 은 대입문이 쓰일 수 없는 위치에서 변수에 값을 대입할 수 있으므로 유용하다
