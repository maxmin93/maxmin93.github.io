...
2022.9 프론트 개발 플랫폼별 검색건수 (관심도 국내/세계)

1.  "asp 개발자 구인" --> 18만 8천건 (55%) 관심도 6/10
2. "react 개발자 구인" --> 8만 9천건 (26%) 관심도 71/75 <= 차이 없음
3. "vue 개발자 구인" --> 4만 6천건 (13%) 관심도 28/48
4. "angular 개발자 구인" --> 2만 건 (6%) 관심도 5/36 <= 차이 심함

> Summary

- 역쉬.. 문서량도, 관심도도 제일 높은 프레임워크는 React
  - 올 초부터 전세계적으로도 검색량에서 React 가 압도적으로 치고 나감 (무슨 일?)
- ASP 는 문서는 많아도 관심도는 React 의 1/10 이하
  - 인기는 없지만 웹검색을 통하지 않고도 인력 수급이 가능한걸로 보임 (기존 고인물들)
- 그나마 angular 는 수학 용어로 혼용되어 수요나 관심이 없다고 보면 된다.
  - 재밋는건 전세계를 놓고 보면 Angular 가 Vue 를 추월할 기세임

> React 를 해야함. 적어도 협업할 정도는. 국내도 국외도 인기 Top1
